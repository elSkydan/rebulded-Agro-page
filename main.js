...
function initForm() {
  ...
  const citySelect = document.getElementById('form-city-id');
  ...
  async function loadCities() {
    ...
    try {
      const res = await fetch('/api/cities/public');
      ...
      console.error('[form] Failed to load cities:', err.message);
  ...
  form.addEventListener('submit', async e => {
    ...
    try {
      ...
      const calcArea = document.getElementById('calc-area-input');
      const outskirts = document.getElementById('outskirts-cb');
      const serviceType = calcService?.value || document.getElementById('form-service')?.value || '';
      const area = Number(calcArea?.value);
      const outOfCity = Boolean(outskirts?.checked);
      ...
      alert('Не вдалося надіслати заявку. Перевірте поля та спробуйте ще раз.');
    }
  });
}
